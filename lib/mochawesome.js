...
function Mochawesome (runner, options) {
  ...
  runner.on('pass', function (test) {
    ...
    if (config.passedScreenshot){
      saveScreenshotToFile(config.reportScreenshotDir + '/' + test.fullTitle().replace(/[^\w_\-]/g, '_').toLowerCase().substr(0,config.screenshotMaxFileLenght) + '.png');
    }
  });
...
function cleanTest (test) {
  ...
  if (err && err.stack) {
      var err = test.err ? _.pick( test.err, ['name', 'message', 'expected', 'actual', 'stack', 'scr'] ) : test.err,
          scr = '<img src="./screenshots/' + test.fullTitle().replace(/[^\w_\-]/g, '_').toLowerCase().substr(0,process.env['MOCHAWESOME_MAXFILELENGHT']) + '.png">';
      cleaned.scr = scr,
      cleaned.err = err,
      err.scr = scr;
