  ...
  runner.on('end', function () {
    try {
      if (!endCalled) {
        ...
        var passPercentage;
        if(obj.stats.tests && reporterOpts.runPercentage) {
          passPercentage = Math.round((obj.stats.passes / (obj.stats.tests - obj.stats.pending))*1000)/10;
        }
        else {
          passPercentage = Math.round((obj.stats.passes / (obj.stats.testsRegistered - obj.stats.pending))*1000)/10;
        }
        ...
        obj.stats.executedTests = obj.stats.tests - obj.stats.pending;
