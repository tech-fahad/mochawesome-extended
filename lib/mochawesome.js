...
function Mochawesome (runner, options) {
  ...
  runner.on('pass', function (test) {
    ...
    if (config.passedScreenshot){
      saveScreenshotToFile(config.reportScreenshotDir + '/' + (test.timer._idleStart.toString() + '_' + test.fullTitle().replace(/[^\w_\-]/g, '_').toLowerCase()).substr(0,config.screenshotMaxFileLenght) + '.png');
    }
  });
...
function cleanTest (test) {
  ...
  var err = test.err ? _.pick( test.err, ['name', 'message', 'expected', 'actual', 'stack', 'scr'] ) : test.err,
    scr = '<img src="./screenshots/' + (test.timer._idleStart.toString() + '_' + test.fullTitle().replace(/[^\w_\-]/g, '_').toLowerCase()).substr(0,process.env['MOCHAWESOME_MAXFILELENGHT']) + '.png">';
