...
function Mochawesome (runner, options) {
  ...
  runner.on('pass', function (test) {
    ...
    if (config.passedScreenshot) {
      ...
      var fileName = (config.shortScrFileNames) ? idleStart : (idleStart + test.fullTitle().replace(/[^\w_\-]/g, '_').replace(/_+/g, '_').toLowerCase()).substr(0,config.screenshotMaxFileLenght);
...
function cleanTest (test) {
  ...
  var idleStart = (test.timer) ? test.timer._idleStart.toString() + '_' : '';
  var fileName = (config.shortScrFileNames) ? idleStart : (idleStart + test.fullTitle().replace(/[^\w_\-]/g, '_').replace(/_+/g, '_').toLowerCase()).substr(0,process.env['MOCHAWESOME_MAXFILELENGHT']);
