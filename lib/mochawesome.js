...
function Mochawesome (runner, options) {
  ...
  runner.on('pass', function (test) {
    ...
    if (config.passedScreenshot) {
      var idleStart = (test.timer) ? test.timer._idleStart.toString() : '';
      var fileName = (config.shortScrFileNames && test.timer) ? idleStart : (idleStart + '_' + test.fullTitle().replace(/[^\w_\-]/g, '_').replace(/_+/g, '_').toLowerCase()).substr(0,config.screenshotMaxFileLenght);
      saveScreenshotToFile(config.reportScreenshotDir + '/' + fileName + '.png');
    }
  });
...
function cleanTest (test) {
  ...
  var idleStart = (test.timer) ? test.timer._idleStart.toString() : '';
  var fileName = (process.env['MOCHAWESOME_SHORTSCRFILENAMES'] === 'true' && test.timer) ? idleStart : (idleStart + '_' + test.fullTitle().replace(/[^\w_\-]/g, '_').replace(/_+/g, '_').toLowerCase()).substr(0,process.env['MOCHAWESOME_MAXFILELENGHT']);

  var err = test.err ? _.pick( test.err, ['name', 'message', 'expected', 'actual', 'stack', 'scr'] ) : test.err,
    scr = '<img src="./screenshots/' + fileName + '.png">';
